 ...
 * configdeki timeout için verilen sürede test sonuclanmaz ise sonuc fail olur
 * 
 */
...
test.only('Tag Usage Test 1', async ({ page }) => { 
  ...
  test.fail('Tag Usage Test 6', async ({ page }) => { //bu durumda error vermesini bekleriz ama expect()2 ===1 olsa pass olmasını bekleriz
  ...
  test('Tag Usage Test 8', async ({ page }) => {
      //test.setTimeout(5000) eger manuel timeout girmek istersek burdan bu teste özel timeout girebiliriz.  
      test.slow();
